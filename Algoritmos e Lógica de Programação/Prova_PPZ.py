...
def squirrel_play(temp, is_summer):
    if temp >=60 and temp <=90:
        es = True
    elif is_summer == True and temp > 90 and temp <= 100:
        es = True
    else:
        es = False
    return es
...
def fila_tijolos(n_peq, n_gra, meta):
    tj = False
    if meta%5 == 0 and n_peq + (n_gra*5) >= meta:
        tj = True
    elif meta%5 <= n_peq and int(meta/5) <= n_gra:
        tj = True
    elif meta%5 <= n_peq and meta <= (n_gra*5)+n_peq:
        tj = True
    return tj
...
def busca(frase, palavra):
    i = 0
    c = 0
    while frase.find(palavra, i) != -1:
        if frase.find(palavra, i) != -1:
            c = c +1
            i += abs(i - frase.find(palavra, i)) + 1
    return c
...
def inip2(n):
    p = 0
    while str(2**p)[0:len(str(n))] != str(n):
        p = p +1
    return p
